...
def get_background_qr_code(length_qr: int, path_to_download: Path):
    try:
        ...
        background = Image.open(path_to_download).crop(crop_area).resize(
            (length_qr, length_qr)).convert("RGBA")
        return background
    except Exception as e:
        print("Error", e)
        return None
...
def gen_qr_code(text_for_qr: str, path_to_download: Path, path_to_save: Path):
    ...
    idraw.rectangle((length_qr, 0, length_qr -
                     coeff * 9, coeff * 9), fill=white_1)
    idraw.rectangle(
        (0, length_qr, coeff * 9, length_qr - coeff * 9), fill=white_1)
    ...
    idraw.rectangle((length_qr - coeff * 8, coeff,
                     length_qr - coeff, coeff * 2), fill=black_2)
    idraw.rectangle((coeff, coeff * 7, coeff * 8, coeff * 8), fill=black_2)
    idraw.rectangle((length_qr - coeff * 8, coeff * 7,
                     length_qr - coeff, coeff * 8), fill=black_2)
    idraw.rectangle((coeff, length_qr - coeff * 7, coeff *
                     8, length_qr - coeff * 8), fill=black_2)
    idraw.rectangle((coeff, length_qr - coeff * 2, coeff *
                     8, length_qr - coeff), fill=black_2)
    ...
    idraw.rectangle((length_qr - coeff * 3, coeff * 3,
                     length_qr - coeff * 6, coeff * 6), fill=black_2)
    idraw.rectangle((coeff * 3, length_qr - coeff * 3, coeff *
                     6, length_qr - coeff * 6), fill=black_2)
    idraw.rectangle((coeff, coeff, coeff * 2, coeff * 8), fill=black_2)
    idraw.rectangle((coeff * 7, coeff, coeff * 8, coeff * 8), fill=black_2)

    idraw.rectangle((length_qr - coeff, coeff, length_qr -
                     coeff * 2, coeff * 8), fill=black_2)
    idraw.rectangle((length_qr - coeff * 7, coeff, length_qr -
                     coeff * 8, coeff * 8), fill=black_2)

    idraw.rectangle((coeff, length_qr - coeff, coeff * 2,
                     length_qr - coeff * 8), fill=black_2)
    idraw.rectangle((coeff * 7, length_qr - coeff, coeff *
                     8, length_qr - coeff * 8), fill=black_2)
