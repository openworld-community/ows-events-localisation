...
class EventOnPoster(dict):
    ...
    durationInSeconds: int = 1000  # а это вообще зачем? можно использовать формат timedelta

    location: dict = field(init=False, repr=True, default=dict[str, str])
    country: str = field(init=True, repr=False, default='Limonia')
    city: str = field(init=True, repr=False, default='default_city')

    image: str = 'this_is_image_url'  # может быть переименовать поле типа на image_url ?
    ...
    timezone: dict = field(init=False, repr=True, default=dict[str, str])
    timezoneName: str = field(init=True, repr=False, default='time_zone')
    timezoneOffset: str = field(init=True, repr=False,
                                default='timezone_offset')  # может быть тоже поменять формат на более удобный?
    ...
    def __post_init__(self):
        ...
        self.timezone = dict(timezoneName=self.timezoneName, timezoneOffset=self.timezoneOffset)
...
class PostEventPayload(dict):
    ...
    location: dict = field(init=False, repr=False, default=dict[str, str])
    country: str = field(init=True, repr=False, default='Limonia')
    city: str = field(init=True, repr=False, default='default_city')
    ...
    timezone: dict = field(init=False, repr=False, default=dict[str, str])
    timezoneName: str = field(init=True, repr=False, default='time_zone')
    ...
    def __post_init__(self):
        ...
        self.location = dict(country=self.country, city=self.city)
        self.timezone = dict(timezoneName=self.timezoneName, timezoneOffset=self.timezoneOffset)
        self.event = dict(title=self.title,
