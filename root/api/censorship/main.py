...
from root.api.censorship.types.validator_types import PostEventPayload, EventValidatorResult  # , EventOnPoster
from datetime import datetime
import re

EXCLUDE_LIST: list[str] = ['хуй', 'негр', 'ниггер',
                           'нигер', 'пизда', 'ебать',
                           'бля', 'ебанутый', 'митинг']
...
class EventsValidator:
    def validate_event(self, event: PostEventPayload) -> EventValidatorResult:
        ...
        if event is None:
            result = EventValidatorResult(isValid=False, errors=[EventValidatorErrorTypes.EVENT_IS_NOT_DEFINED])
            return result

        event = event.get("event")  # get data from event
        prevalidation_event_result: EventValidatorResult = self.prevalidation_event(event)

        if not prevalidation_event_result.isValid:
            return prevalidation_event_result

        title_is_ok = self.validate_string(event.get("title"))
        description_is_ok = self.validate_string(event.get("description"))
    ...
    def prevalidation_event(self, event: PostEventPayload.event) -> EventValidatorResult:

        if event is None:  # Двойная проверка нам точно нужна?
            result = EventValidatorResult(isValid=False, errors=[EventValidatorErrorTypes.EVENT_IS_NOT_DEFINED])
            return result

        errors: list[str] = self.validate_title(event.get('title')) + \
    ...
    def validate_string(self, line: str) -> bool:
        ...
        for word in words:
            if not word.isdigit():  # сразу выбрасываем числа
                if len(word) > 2:  # из двух букв нельзя составить гадость, а цикл ускорит
    ...
    def validate_date(self, date: int) -> list[str]:
        errors: list[str] = []
        if date is None or date == '':
            errors.append(EventValidatorErrorTypes.START_DATE_IS_NOT_DEFINED)
        elif date < int(datetime.now().strftime('%Y%m%d')):  # date = int с 1970 года unix timestamp
            errors.append(EventValidatorErrorTypes.START_DATE_IS_IN_THE_PAST)
        return errors

    def validate_duration(self, duration: int) -> list[str]:
        errors: list[str] = []
        if duration is not None and duration < 0:
            errors.append(EventValidatorErrorTypes.DURATION_IS_NEGATIVE)
        return errors

    def validate_location(self, location: dict[str, str]) -> list[str]:
        errors: list[str] = []

        if location is None or location == {}:
            errors.append(EventValidatorErrorTypes.LOCATION_IS_NOT_DEFINED)
        else:
            if location.get('country') is None or location.get('country') == '':  # Нужна проверка по списку
                errors.append(EventValidatorErrorTypes.COUNTRY_IS_NOT_DEFINED)
            if location.get('city') is None or location.get('city') == '':  # Нужна проверка по списку
